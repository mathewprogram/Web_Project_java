...
public class Methods {
    ...
    public static List<String> showUsers(Connection connection) {
        List<String> users_al = new ArrayList<>();
    ...
    public static List<String> showProffesors(Connection connection) {
        List<String> users_al = new ArrayList<>();
        ...
        try {
            ...
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                String proffesor = rs.getString("nombre");
                users_al.add(proffesor);
            }
        } catch (SQLException e) {
            JOptionPane.showMessageDialog(null, "Error not expected", "Error", JOptionPane.ERROR_MESSAGE);
        }
    ...
    public static List<String> showAlumno(Connection connection) {
        List<String> users_al = new ArrayList<>();
    ...
    public static List<String> showModulo(Connection connection) {
        List<String> users_al = new ArrayList<>();
    ...
    public static List<String> showGrupo(Connection connection) {
        List<String> users_al = new ArrayList<>();
        String query = "SELECT G.nombre AS grupo, A.nombre AS delegado "
                + "FROM Grupo G "
                + "LEFT JOIN Alumno A ON G.delegado_id = A.expediente "
                + "WHERE G.delegado_id IN (1, 11, 21, 31);";

        try (PreparedStatement ps = connection.prepareStatement(query); ResultSet rs = ps.executeQuery()) {
            // Procesar resultados de la consulta
            while (rs.next()) {
                String grupo = rs.getString("grupo");
                String delegado = rs.getString("delegado") != null ? rs.getString("delegado") : "Sin delegado";
                users_al.add(grupo + ": Delegado - " + delegado);
            }
        } catch (SQLException e) {
            JOptionPane.showMessageDialog(null, "Error not expected: " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
